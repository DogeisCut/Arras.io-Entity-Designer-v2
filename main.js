  ...
  let useMiter = false;
	...
	if (!shape) {
		...
		for (let [x, y] of shape) {
			context.lineTo(
				centerX + size * (x * ax - y * ay),
				centerY + size * (y * ax + x * ay),
			);
		}
	} else if (0 < shape) {
		...
		for (let i = 0; i < shape; i++) {
			...
			context.lineTo(
				centerX + size * Math.cos(a + angle),
				centerY + size * Math.sin(a + angle),
			);
		}
	} else if (0 > shape) {
		if (0 === shape % 2) angle += Math.PI / shape;
...
function drawGun(
  ...
  let recoil = 0; //when a gun shoots, it recoils back for like half a second.
    ...
    x +
    size *
      (offset * Math.cos(direction + angle) +
        (length / 2 - recoil) * Math.cos(angle));
	y =
    y +
    size *
      (offset * Math.sin(direction + angle) +
        (length / 2 - recoil) * Math.sin(angle));
	...
	context.beginPath();
	context.moveTo(
		x + halflength * ax - halfwidthclose * ay,
		y + halflength * ay + halfwidthclose * ax,
	);
	context.lineTo(
		x - halflength * ax - halfwidthfar * ay,
		y - halflength * ay + halfwidthfar * ax,
	);
	context.lineTo(
		x - halflength * ax + halfwidthfar * ay,
		y - halflength * ay - halfwidthfar * ax,
...
function mixColors(color1, color2, amount) {
	// The mix color function, input 1 and 2 are swapped.
  let c1 = color1.match(
    /^#?([0-9a-angle]{2})([0-9a-angle]{2})([0-9a-angle]{2})$/i,
  );
	let c2 = color2.match(
		/^#?([0-9a-angle]{2})([0-9a-angle]{2})([0-9a-angle]{2})$/i,
	);
	let c = [
		Math.round(
			parseInt(c1[1], 16) * amount + parseInt(c2[1], 16) * (1 - amount),
		),
		Math.round(
			parseInt(c1[2], 16) * amount + parseInt(c2[2], 16) * (1 - amount),
		),
		Math.round(
			parseInt(c1[3], 16) * amount + parseInt(c2[3], 16) * (1 - amount),
		),
	];
...
function getColorDark(givenColor) {
	// Gets the darker color of centerX given color, its exactly how the game does it but input 1 and 2 are swapped
